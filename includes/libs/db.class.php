...
elseif(extension_loaded('mysqli') && $nomysqli==false) {
    class DB {
		...
		function update_array($table,$array,$where){
		    $q = "UPDATE `$table` SET";
		    foreach($array as $key => $value) {
		        $q .= " `{$key}` = '".$value."',";
		    }
		    $q = rtrim($q, ",");
		    $q .=" WHERE `{$where}`='{$array[$where]}' ";
		    if(mysqli_query($this->link,$q))    return mysqli_affected_rows($this->link);
		    return false;
		}
		function error(){
	...
	class DB {
		...
		function update_array($table,$array,$where){
		    ...
		    $q = rtrim($q, ",");
		    $q .=" WHERE `{$where}`='{$array[$where]}' ";
		    if(mysql_query($this->link,$q))    return mysql_affected_rows($this->link);
		    return false;
		}
		function error(){
